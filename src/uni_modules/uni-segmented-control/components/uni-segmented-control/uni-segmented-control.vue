				...
				<text
				  :style="{
				    color: index === currentIndex 
				      ? (styleType === 'text' ? activeColor : '#fff') // 选中时逻辑不变
				      : (styleType === 'text' ? inactiveTextColor : inactiveTextColor) // 未选中时用新增属性
				  }"
	...
	export default {
		...
		props: {
			...
			inActiveColor: {
			 ...
			 inactiveTextColor: { // 未选中文字颜色（新增）
			    type: String,
			    default: '#000000' // 默认黑色，可外部覆盖
			  },
			styleType: {
				type: String,
				default: 'button'
